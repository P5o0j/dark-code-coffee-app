...
function CoffeeSide() {
  ...
  function handleAddToCart() {
    toast.success(
      `Your Order is  ${fakeCoffee.id}, ${fakeCoffee.title} ${selectedBagSize}, ${selectedGround}`
    );
  }

  let price = 7.99;
  if (selectedBagSize === 500) {
    setPrice(9.99);
  } else if (selectedBagSize === 1000) {
    setPrice(14.99);
  } else price;
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-20 border lg:divide-x py-5 px-5 lg:px-10 mb-24 rounded-lg bg-black">
        ...
        <div className="lg:pl-10">
          ...
          <div className="py-6 flex justify-start items-end">
            ...
            <span className="mb-5 px-8 text-4xl">£{price}</span>
          </div>
          {/* BUTTONS */}
